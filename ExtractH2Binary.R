...
path<-paste0("../OUTPUT/Simulations/Simul1Binary25",i,j,"_",caus,".h2.log") 
data<-readLines(path)
temp<-sub("h2:", "", data[grep("h2:",data)])   
h2mat1[i,j]<-as.numeric(strsplit(temp, " +")[[1]][4])

path<-paste0("../OUTPUT/Simulations/Simul2aBinary25",i,j,"_",caus,".h2.log") 
data<-readLines(path)
temp<-sub("h2:", "", data[grep("h2:",data)])   
h2mat2a[i,j]<-as.numeric(strsplit(temp, " +")[[1]][4])

path<-paste0("../OUTPUT/Simulations/Simul2bBinary25",i,j,"_",caus,".h2.log") 
data<-readLines(path)
temp<-sub("h2:", "", data[grep("h2:",data)])   
...
ggsave(plot=plot,height=6,width=6,dpi=200, filename=paste0("../OUTPUT/FIGURES/H2SimulationsBinary25",caus,".pdf"))

scen1<-read.table(paste0("../TEMP/Simul1ResultsBinary25",caus,".txt"),sep="\t")
...
scen2a<-read.table(paste0("../TEMP/Simul1Results2aBinary25",caus,".txt"),sep="\t")
scen2b<-read.table(paste0("../TEMP/Simul1Results2bBinary25",caus,".txt"),sep="\t")
...
ggsave(plot=plot,height=6,width=6,dpi=200, filename=paste0("../OUTPUT/FIGURES/PointEstSimulationsBinary25",caus,".pdf"))
}
