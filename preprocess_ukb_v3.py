...
### DEFINE FILE PATHS
#construction_fp = "/projects/0/Galama/andriesm/GWAS_PIPELINE/Regenie_pipeline/Regenie_software/Phenotypes_rgses"
construction_fp = "/projects/0/Galama/UKB_WGWAS/INPUT/PHENOTYPES"
crosswalk_fp = "/projects/0/Galama/andriesm/GWAS_PIPELINE/INPUT/ukb_v3_newbasket.s487395.crosswalk"
ukb_fp = "/projects/0/Galama/andriesm/GWAS_PIPELINE/OUTPUT/basket2005284.ukb46101_refresh_pheno.csv.gz"
sibs_fp="/projects/0/Galama/svalten/UKBKin/UKB2_FS_withfam.txt"
individual=True
filtered=True
nosibs=False 
#### READ DATA
...
ukb_cols = ["f.31",		# Sex
            ...
            "f.845",		# Age completed full time education 
			"f.738",		# Household income
			"f.2178",		# Health rating
			"f.2887",		# Cigs per day
			"f.2754",		# Age first birth (female)
			"f.2867",		# Age started smoking
			"f.21001",	# Body Mass Index (BMI)
            "f.22003", #Heterozygosity rate 
			"f.20116",	# Smoking cessation
			"f.41202", "f.41204",
						# Type I Diabetes
						# Type II Diabetes
						# Severe Obesity
						...
						# Alzheimer's Disease
			"f.20018",	# Prospective memory test 
			"f.6150",		# High blood pressure
						# Stroke
			"f.137",		# Treatments / medications taken 
			"f.2020",		# Loneliness 
			"f.20116",	# Smoke Initiation
			"f.20126",	# Unipolar Depression
			"f.1200",		# Insomnia symptoms
  			"f.20002", 	# Arthritis 
  			"f.135",		# Non-cancer illnesses 
  			"f.20421",	# Anxiety 
  			"f.50",		# Height 
 			"f.22127",	# Asthma 
 			"f.20127",	# Neuroticism Score 
			"f.2000",		# Feeling Worry
			"f.40006",	# Breast Cancer		
			"f.30690",	# Cholesterol
			"f.2405", 	# Number of children fathered (male) 
			"f.2453",		# Cancer
			"f.2040",		# Risk taking behaviour
			"f.41270",	# Alzheimer's
			"f.6148",		# Cataract
			"f.2247",		# Hearing difficulty/problems
			"f.2734",		# Number of live births (female) 
			"f.20001",	# Number of live births (female) 	
			"f.20458",	# Positive Affect
			"f.20460",	# Life Satisfaction	  	
			"f.20016", 	# Cognitive Performance
			"f.894", "f.914", #"f.884", "f.904",		
			# Moderate-to-vigorous physical activity
			"f.2946", "f.1807", "f.1845", "f.3526",
			# Parental longevity
			"f.20514", "f.20510", "f.20517" , "f.20519", "f.20511", "f.20507", "f.20508", "f.20518", "f.20513"      
			# Depressive symptoms
			]
...
ukb_iterator = pd.read_csv(ukb_fp, engine="python", encoding = "ISO-8859-1",
					...
					usecols=lambda col: re.search("f.eid|^" + "\.|^".join(ukb_cols) + "\.", col), chunksize=50000)
...
# Read siblings data
sibs = pd.read_csv(sibs_fp, sep=" ")

# Read crosswalk file if present
if "crosswalk_fp" in locals():
	crosswalk = pd.read_csv(crosswalk_fp, delim_whitespace=True, names=["genetic_ID", "IID"])
...
if filtered:
    ...
    N2=len(ukb)
    print("removed",N1-N2,"individuals that were not of white british ancestry")
    print(len(ukb))
    N1=len(ukb)
    ukb = ukb[(ukb["22005-0.0"])<=0.02]
    N2=len(ukb)
    print("removed",N1-N2,"individuals due excessive missingness (>2%)")
    print(len(ukb))
    sd=ukb["22003-0.0"].std()
    m=ukb["22003-0.0"].mean()
    N1=len(ukb)
    ukb = ukb[(ukb["22003-0.0"]>=m-3*sd)&(ukb["22003-0.0"]<=m+3*sd)]
    N2=len(ukb)
    print("removed",N1-N2,"with outlying heterozygosity")
    print(len(ukb))
    N1=len(ukb)
    ukb = ukb[(ukb["22001-0.0"]==ukb["31-0.0"])]
    N2=len(ukb)
    print("removed",N1-N2,"for whom self-reported sex does not equal genetic sex")
    print(len(ukb))
...
if nosibs:
    ...
    #sibssub=sibs.groupby('fam').apply(lambda x: x.sample(1)).reset_index(drop=True)
    #merge with genetic missingness data
    sibs = sibs.merge(ukb[["eid","22005-0.0"]], left_on="ID", right_on="eid")
    sibssub=sibs.loc[sibs.groupby('fam')['22005-0.0'].idxmin()] #select one sibling from each group with minimum missingness  
    #sibs.groupby('fam').apply(lambda x: x.min("22005-0.0")).reset_index(drop=True)
    sibremove=set(sibs.ID).difference(sibssub.ID)
    #non_sibs = set(ukb.eid).difference(sibremove)
    N1=len(ukb)
    ukb = ukb[~ukb.eid.isin(sibremove)]
    N2=len(ukb)
    print("removed",N1-N2,"individuals because they were part of sibling pair")
...
#diagnoses.to_csv("diagnosis_codes_full.csv")
...
# mapping obtained from Emil & Dilnoza
educ_dict = {1: 20, 2: 13, 3: 10, 4: 10, 5: 19, 6: 15, -7: 7}
# convert all "qualification" columns to years of education
educ_cols = [col for col in ukb.columns if re.search("^6138-", col)]
ukb_educ = ukb[educ_cols].applymap(lambda x: educ_dict.get(x))
#ukb_educ
#when respondents report NVQ, change to age left education - 5
age_left_cols = [col for col in ukb.columns if re.search("^845-", col)]
left_dict = {-1: np.nan, -2: np.nan, -3: np.nan}
ukb_age_left = ukb[age_left_cols].applymap(lambda x: left_dict.get(x, x))
ukb_age_left
ukb_age_left = ukb_age_left.max(axis=1)

#get positions at which value == 19, replace with age at which left full time educ. - 5

for i in educ_cols:
    list = ukb_educ[i][(ukb_educ[i]==19.0)].index
    b=ukb_age_left[list]
    drop=b[(b<=12)].index
    b[(b>=24)]=24
    ukb_educ[i][list]=b-5
    ukb_educ[i][(ukb_educ[i]<7)]=np.nan


# for each wave, calculate the max, then keep first wave if available
for i in range(0,3):
	i = str(i)
	tmp = ukb_educ.filter(regex="-"+i)
	ukb_educ["educ"+i] = tmp.max(axis=1)

# keep first non-missing wave and put back in ukb dataframe
ukb["educYears"] = ukb_educ.filter(regex="educ").bfill(axis=1).iloc[:,0]

print(ukb["educYears"].describe()) 
...
# only need to re-code negative values, others are in year units already
ageSmoke_dict = {-1: np.nan, -3: np.nan}
ageSmoke_cols = [col for col in ukb.columns if re.search("^2867-", col)]
ukb[ageSmoke_cols] = ukb[ageSmoke_cols].applymap(lambda x: ageSmoke_dict.get(x, x))
# use first available observation as there shouldn't be inconsistencies
ukb["ageSmoke"] = ukb[ageSmoke_cols].bfill(axis=1).iloc[:,0]
ageSmoke = ukb.dropna(subset=["ageSmoke"])[["FID", "IID", "ageSmoke"]]
...
# necessary to convert to numeric since there were some outliers messing up the later arithmetic
...
# necessary to convert to numeric since there were some outliers messing up the later arithmetic
ukb_actModVig = ukb_actModVig.apply(pd.to_numeric, errors="coerce")
...
##Write a list of genetic sample IDs and f.eids that make the analytic sample:
list = crosswalk.merge(ukb["IID"])
list = pd.DataFrame(data=list)
list["FID"]=list['genetic_ID'].str.replace("sample_","")
list["FID"]=pd.to_numeric(list["FID"])+1
geneIDlist=list[["FID","genetic_ID"]]
EIDlist=list["IID"]

geneIDlist.to_csv("../INPUT/GeneIDs.txt", sep="\t", index=False, na_rep="NA",header=False)
EIDlist.to_csv("../INPUT/EIDs.txt", sep="\t", index=False, na_rep="NA",header=False)
#### WRITE DATA
...
def write_pheno(df, fp, crosswalk=None):
	...
	if crosswalk is None:
		return
	joined = df.merge(crosswalk, how="inner", on="IID").drop(["FID", "IID"], axis=1)
...
# write all data
pheno_dct = \
	{"dpw": dpw, "educYears": educ, "householdIncome": householdIncome, "healthRating": health, "maxcpd": maxcpd, "ageFirstBirth": ageFirstBirth, "ageSmoke": ageSmoke, 
	...
	"wellBeingSpectrum": wellBeingSpectrum, "ageParents90th": ageParents, "actModVig": actModVig}

# temp
#pheno_dct = {"t2d": t2d}
#ukb[(ukb['IID'].isnull())]

#ukb=ukb.astype({'FID':'int'}, errors='ignore')
...
if individual:
	...
	for name, df in pheno_dct.items():
		write_pheno(df, os.path.join(construction_fp + "/" + name + "_pheno"), crosswalk)
