...
#GWAScat<-read.table("../OUTPUT/gwascatalog.txt",header=TRUE,sep="\t",fill=TRUE)
...
GWAScat$Trait[grep("anorexia nervosa, obsessive-compulsive disorder",GWAScat$Trait)]<-
                                    "anorexia nervosa"

GWAScat[GWAScat$SNP=="rs4380187",]
...
N<-t(sapply(N,"[", c(1,2,4,6,13)))
N<-apply(N,2,gsub,pattern=",",replacement="")
N<-apply(N,2,as.numeric)
N<-apply(N,1,sum,na.rm=TRUE)
GWAScat$N<-N
GWAScat<-GWAScat[rowSums(is.na(GWAScat)) != ncol(GWAScat),]
GWAScat<-GWAScat[GWAScat$N>=50000,]

tophits<-read.table("../OUTPUT/TABLES/TopHitsGWASOnTheWeights.csv",sep=",",header=TRUE)


MAFdata<-read.table(paste0("../TEMP/MAF/WeightedMAFBootstrapTopHits","1",".txt"), sep="\t",header=TRUE)
data<-read.table(paste0("../TEMP/GWASonTheWeightsAroundTopHits","1",".P1.assoc.linear"),header=TRUE)

bimTMP<-read.table("../TEMP/PLINKFILES/UKBGWASAroundTophitsQC1LD0.1.bim")
...
LDInfo<-read.table("../TEMP/PLINKFILES/UKBGWASAroundTophitsQC1.ld",header=TRUE)

for (chr in c(2,4,6,13)){
  #MAFdataTMP<-read.table(paste0("../TEMP/MAF/WeightedMAFBootstrapTopHits",chr,".txt"), sep="\t",header=TRUE)
  dataTMP<-read.table(paste0("../TEMP/GWASonTheWeightsAroundTopHits",chr,".P1.assoc.linear"),header=TRUE)
  
  bimTMP<-read.table(paste0("../TEMP/PLINKFILES/UKBGWASAroundTophitsQC",chr,"LD0.1.bim"))
  bimTMP<-bimTMP[c("V1","V2","V4","V5","V6")]
  names(bimTMP)<-c("CHR","SNP","BP","A1","A2")
  print(NROW(dataTMP))
  ...
  LDInfoTMP<-read.table(paste0("../TEMP/PLINKFILES/UKBGWASAroundTophitsQC",chr,".ld"),header=TRUE)
...
for (i in 1:NROW(tophits)){
  ...
  BPHigh<-BP+500000
  BPLow<-BP-500000
  tmpData<-data[data$BP>=BPLow&data$BP<=BPHigh,]
  ...
  #when there are duplicate triats, keep most recent:
  tmpData<-tmpData[order(tmpData$Date),]
  NROW(tmpData)
  ...
  png(paste0("../OUTPUT/FIGURES/GWASOnTheWeightsMAF_LocalManhattan",hit,".png"), width=800, height=800, res=150)
  ...
  #print(p)
  ggsave(paste0("../OUTPUT/FIGURES/GWASOnTheWeights_LocalManhattanAnnotated",hit,".pdf"),width=1800, height=1200, dpi=288, units="px")
  #dev.off()
}