#Import top hits for breast cancer:
...
tophits<-read.table("../OUTPUT/TABLES/DifferentSNPsWGWASSigBreastCancer.csv",sep=",",header=TRUE)

Pheno <- fread("../TEMP/BreastCancer.resid.txt",header=TRUE,sep="\ ")
PhenoResid<-"BreastCancer.resid"
PhenoWeightResid<-"BreastCancer.Wresid"
...
GenoData<-as.data.frame(GenoData[,c(tophits$SNP)])
...
vars<-c("Education","region","birthyear","sex","carsnoc","HealthSelfReport","Empstat","Tenure","SingleHousehold","Educationregion")
labels<-c("Unweighted","All Variables","Education","Region","Year of Birth","Sex","Number of Cars","Self-reported Health",
          "Household Tenure","Employment Status","Single Household","Education*Region")
color<-c("Unweighted","Weighted","Weighted, selected variables","Weighted, selected variables","Weighted, selected variables"
         ,"Weighted, selected variables","Weighted, selected variables","Weighted, selected variables","Weighted, selected variables",
         "Weighted, selected variables","Weighted, selected variables","Weighted, selected variables")
...
controls<-"Birthyear+GeneBatch+PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10+PC11+PC12+PC13+PC14+PC15+PC16+PC17+PC18+PC19+PC20"


#Replicate original results:
#Consider SNP with lowest p-value: rs2306412 
SNP<-"rs2306412"
...
assocNoCont<-lm_robust(formula=formula(paste("BreastCancer","~",SNP)),se_type="HC0",data=Data, return_vcov=FALSE)
print(summary(assocNoCont))
robresults[3,]<-summary(assocNoCont)$coefficients[SNP,1:4]


assocWTNoCont<-lm_robust(formula=formula(paste("BreastCancer","~",SNP)),se_type="HC0",data=Data,weights=Data$LassoWeight, return_vcov=FALSE)
...
assoclogit<-glm(formula=formula(paste("BreastCancer","~",SNP,"+",controls)),data=Data, family="binomial")
print(summary(assoclogit))
robresults[5,]<-summary(assoclogit)$coefficients[SNP,1:4]


assocWTlogit<-glm(formula=formula(paste("BreastCancer","~",SNP,"+",controls)),data=Data,weights=Data$LassoWeight, family="binomial")
print(summary(assocWTlogit))
robresults[6,]<-summary(assocWTlogit)$coefficients[SNP,1:4]

#test robustness against some other specifications:
assoclogitNoCont<-glm(formula=formula(paste("BreastCancer","~",SNP)),data=Data, family="binomial")
print(summary(assoclogitNoCont))
robresults[7,]<-summary(assoclogitNoCont)$coefficients[SNP,1:4]

assocWTlogitNoCont<-glm(formula=formula(paste("BreastCancer","~",SNP)),data=Data,weights=Data$LassoWeight, family="binomial")
...
ggplot(results, aes(x=labels, y=Estimate,color=color)) + geom_point() + geom_errorbar(aes(ymin=`CI Upper`, ymax=`CI Lower`)) +
  ...
  theme(axis.text.x = element_text(angle = 90)) + ylim(-.025,.005) 
ggsave(paste0("../OUTPUT/FIGURES/OneVarBreastCancer",SNP,".pdf"))
