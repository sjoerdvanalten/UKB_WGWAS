...
#args<-c("BreastCancerFemaleChr","BreastCancer","../TEMP/oncoarray_bcac_public_release_oct17_clean.txt.hapmap","SNP","a0","a1","BETA","SE","../TEMP/GWASSum/BreastCancerChr","../TEMP/BreastCancer.Female.resid.txt","BreastCancerFemale")
...
if (length(args)>=12){
  OutPrefix<-as.character(args[12])
}else{
  OutPrefix<-PhenoName
}
#Results<-read.table(paste0("../TEMP/ChromResults/GWAS/",Input,1,".tab"),sep="\t",header=TRUE)
Results<-read.table(paste0("../TEMP/ChromResults/GWAS/",Input,1,".tab"),header=TRUE)
...
write.table(Results[c("SNP","CHR","BP","A1","A2","BETA","SE","P","MAF","N_eff","Z")],paste0("../OUTPUT/GWAS/",OutPrefix,".txt"),quote=FALSE,row.names=FALSE)
write.table(WResults[c("SNP","CHR","BP","A1","A2","BETA","SE","P","MAF","N_eff","Z")],paste0("../OUTPUT/WGWAS/",OutPrefix,".txt"),quote=FALSE,row.names=FALSE)
...
#print("Results significant in GWAS")
#print(Results[Results$P<5*10^(-8)&Results$PLassoWT<5*10^(-8),])
#print("Results significant in WGWAS")
#print(Results[Results$P>=5*10^(-8)&Results$PLassoWT<5*10^(-8),])
n<-nrow(Results[Results$P>=5*10^(-8)&Results$P_LassoWT<5*10^(-8),])
print(paste0("Results significant in WGWAS, but insignificant in GWAS:",n,"in total"))
...
print(Results[Results$P>=5*10^(-8)&Results$P_LassoWT<5*10^(-8),])
...
nrow(Results)
Results<-Results[!is.na(Results$PHausman),]
nrow(Results)


print(head(Results[Results$PHausman<5*10^(-8),]))
print(nrow(Results[Results$PHausman<5*10^(-8),]))

nrow(Results)
indep<-Results[Results$SNP %in% GWASClump$SNP,]
nrow(indep)

n_Haus<-NROW(indep[indep$PHausman<5*10^(-8),])

n<-NROW(indep[indep$PHausman<5*10^(-8)&indep$P>5*10^(-8)&indep$P_LassoWT<5*10^(-8),])

nUniqueSig<-n

print(paste0("Results significant in WGWAS, but insignificant in GWAS that are also significantly different from one another:",
             n,"in total"))


SumResultsOut<-unlist(c(PhenoName,xCorr,round(N_effGWAS),round(N_effW),SEIncrease,yPGWAS,yPWGWAS,n_Haus,nUnique,nUniqueSig))
SumResultsOut<-as.list(SumResultsOut)
#Make p-values for the difference in the SNP:

SumResultsOut<-as.data.frame(SumResultsOut)

names(SumResultsOut)<-c("Phenotype","Correlation","NeffGWAS","NeffIPWGWAS","SEIncrease","yPGWAS5e-08","yPGWAS5e-06","yPWGWAS5e-08","yPWGWAS5e-06","n_Haus","nUnique","nUniqueSig")
write.table(SumResultsOut,paste0("../TEMP/WGWASSum",OutPrefix,".txt"),row.names=FALSE,quote=FALSE,sep="\t")



print(Results[Results$PHausman<5*10^(-8)&Results$P<5*10^(-8),])
if (n>0){
  print(Results[Results$PHausman<5*10^(-8)&Results$P<5*10^(-8),])
  write.table(Results[Results$PHausman<5*10^(-8)&Results$P<5*10^(-8),],file=paste0("../OUTPUT/TABLES/DifferentSNPsGWASSig",
                                                       OutPrefix,".csv"),sep=",",row.names=FALSE,quote=FALSE)
}


n<-NROW(Results[Results$PHausman<5*10^(-8)&Results$P_LassoWT<5*10^(-8),])

print(paste0("Results significant in WGWAS, but insignificant in GWAS that are also significantly different from one another",
             n,"In Total"))

print(Results[Results$PHausman<5*10^(-8)&Results$P_LassoWT<5*10^(-8),])

if (n>0){
  write.table(Results[Results$PHausman<5*10^(-8)&Results$P_LassoWT<5*10^(-8),],file=paste0("../OUTPUT/TABLES/DifferentSNPsWGWASSig",
                                                                                           OutPrefix,".csv"),sep=",",row.names=FALSE,quote=FALSE)
}

nrow(Results)
indepH<-Results[Results$SNP %in% GWASClump$SNP,]
nrow(indepH)

print(head(indepH[indepH$PHausman<5*10^(-8),]))
print(nrow(indepH[indepH$PHausman<5*10^(-8),]))

print(paste0("Independent Results significant in WGWAS, but insignificant in GWAS that are also significantly different from one another:"))

print(indepH[indepH$PHausman<5*10^(-8)&indepH$P<5*10^(-8),])


n<-NROW(indepH[indepH$PHausman<5*10^(-8)&indepH$P<5*10^(-5),])
if (n>0){
  write.table(indepH[indepH$PHausman<5*10^(-8)&indepH$P<5*10^(-5),],file=paste0("../OUTPUT/TABLES/DifferentSNPsGWASSuggestive",
                                                                                   OutPrefix,".csv"),sep=",",row.names=FALSE,quote=FALSE)
}

nrow(Results)
indepH<-Results[Results$SNP %in% WGWASClump$SNP,]
nrow(indepH)


print(paste0("Independent Results significant in WGWAS, but insignificant in GWAS that are also significantly different from one another:"))

print(indepH[indepH$PHausman<5*10^(-8)&indepH$P_LassoWT<5*10^(-8),])


n<-NROW(indepH[indepH$PHausman<5*10^(-8)&indepH$P_LassoWT<5*10^(-5),])
if (n>0){
  write.table(indepH[indepH$PHausman<5*10^(-8)&indepH$P_LassoWT<5*10^(-5),],file=paste0("../OUTPUT/TABLES/DifferentSNPsWGWASSuggestive",
                                                                                        OutPrefix,".csv"),sep=",",row.names=FALSE,quote=FALSE)
}


Results$chisq <- qchisq(Results$PHausman,1,lower.tail=FALSE)
lambda <- median(Results$chisq) / qchisq(0.5,1)

qqResults<-Results$PHausman
qqResults<-c(sample(qqResults[qqResults>0.05],size=floor(0.05*length(qqResults[qqResults>0.05]))),
                 sample(qqResults[qqResults<=0.05&qqResults>0.01],size=floor(0.1*length(qqResults[qqResults<=0.05&qqResults>0.01]))),
             qqResults[qqResults<=0.01])

length(qqResults)

pdf(paste0("../OUTPUT/FIGURES/QQDif",OutPrefix,".pdf"))
#fastqq(Results$PHausman)
qq(Results$PHausman,main=bquote(lambda ~ "=" ~ .(round(lambda,4))))
dev.off()
...
write.table(RegResults,paste0("../TEMP/WGWASReg",OutPrefix,".txt"),row.names=FALSE,quote=FALSE,sep="\t")
...
ggsave(plot=p,height=6,width=6,dpi=200, filename=paste0("../OUTPUT/FIGURES/Scatter",OutPrefix,"TopHits.pdf"))
#dev.off()
